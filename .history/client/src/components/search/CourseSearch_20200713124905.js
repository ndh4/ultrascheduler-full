...
const CourseSearch = ({ scheduleID }) => {
	...
	const displayCourseList = () => {
		if (searchType == "Distribution") {
			return <CourseList scheduleID={scheduleID} department="false" distribution={getDist.value} />
		}
		else {
			return <CourseList scheduleID={scheduleID} department={getDept.value} distribution="false" />
		}
	};
